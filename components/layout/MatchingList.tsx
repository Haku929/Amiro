...
function MatchingCard({ candidate }: { candidate: MatchingResult }) {
    return (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
            ...
            <div className="flex flex-col md:flex-row gap-6 relative z-10">
                ...
                <div className="flex flex-col items-center gap-3 shrink-0 w-32">
                    <div className="w-20 h-20 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                        {candidate.avatarUrl ? (
                    ...
                    <h3 className="text-sm font-bold text-zinc-900 dark:text-zinc-50 text-center leading-tight line-clamp-2 w-full">{candidate.displayName}</h3>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div className="flex items-start justify-between mb-2">
                        <div>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800 inline-block px-2 py-1 rounded-md mb-2">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-lg border border-zinc-100 dark:border-zinc-800">
                            ...
                            <p className="text-sm text-zinc-700 dark:text-zinc-300 line-clamp-2">{candidate.bio || "自己紹介なし"}</p>
                        </div>

                        {/* Persona Summary */}
                        <div className="bg-indigo-50/50 dark:bg-indigo-900/20 p-3 rounded-lg border border-indigo-100/50 dark:border-indigo-800/30">
                            <p className="text-xs text-indigo-400 dark:text-indigo-300 font-bold mb-1">イロ要約</p>
                            <p className="text-sm text-zinc-700 dark:text-zinc-300 line-clamp-2">{candidate.personaSummary || "要約なし"}</p>
                        </div>
                    </div>
                </div>
