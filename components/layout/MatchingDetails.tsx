      ...
      <div className="max-w-5xl mx-auto p-4 lg:p-10 space-y-12">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-stretch justify-center gap-6 md:gap-8">
          ...
          <div className="shrink-0 flex flex-col items-center justify-center py-4 md:py-0">
            {(() => {
              const scoreVal = resonanceScore <= 1 ? resonanceScore * 100 : resonanceScore;
              // 0(青) -> 100(赤) のグラデーション (240 -> 0)
              const hue = Math.max(0, Math.min(240, 240 - (scoreVal * 2.4)));
              const mainColor = `hsl(${hue}, 90%, 55%)`;
              const lightColor = `hsla(${hue}, 90%, 55%, 0.2)`;
              const ringColor = `hsla(${hue}, 90%, 55%, 0.1)`;

              return (
                <div
                  className="relative flex flex-col items-center justify-center w-40 h-40 md:w-48 md:h-48 bg-white dark:bg-zinc-900 rounded-full shadow-lg border-4 transition-colors duration-500"
                  style={{ borderColor: lightColor, boxShadow: `0 0 0 4px ${ringColor}` }}
                >
                  <div
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                    <Zap size={32} fill="currentColor" style={{ color: mainColor }} />
                    <span className="text-xs font-bold uppercase tracking-widest mt-1" style={{ color: mainColor, opacity: 0.8 }}>
                      共鳴スコア
                    </span>
                    <span
                      className="text-5xl md:text-6xl font-black tracking-tighter leading-none mt-1"
          ...
          <div className="w-full md:flex-1 bg-white dark:bg-zinc-900 border border-rose-200 dark:border-rose-900 rounded-3xl p-6 flex flex-col items-center text-center shadow-sm relative overflow-hidden">
            ...
            <div className="w-20 h-20 bg-rose-50 dark:bg-rose-900/20 rounded-full flex items-center justify-center shrink-0 border border-rose-100 dark:border-rose-900/30 mb-4 overflow-hidden">
              {target.avatarUrl ? (
