...
* **3つの「窓口（イロ）」**: ユーザーは一貫した性格を求められません。最大3つまで、異なる自分（イロ）をマッチングの入り口としてストックできます。
...
『Amiro』の心臓部は、各ユーザーが持つ「3つのスロット」にあります。ここには、単なる性格診断の結果ではなく、**「特定の関係性の中で立ち上がったイロ」**の多面的なデータが保存されます。
...
| 項目名 | 内容 | 用途 |
| --- | --- | --- |
| **自己パレット ()** | 自身の性格数値 (Big5準拠の5次元) | 他者の「理想」との照合用 |
| **共鳴パレット ()** | その会話の鏡の Big5（フロントが生成したランダムな 5 次元。日付シードで同一日は同じ値）。マッチングでは「自分が求める理想」として検索に使う。 | 自身の「理想」として検索用 |
| **イロアイコン** | イロアイコン画像のURL | 自身のイロの識別・愛着 |
| **イロ要約文** | 会話から抽出された、そのイロの特徴 | プロフィール画面での表示 |
...
マッチングは**相互共鳴**を採用する。自分→相手（自分の共鳴パレットと相手の自己パレットの類似度）と、相手→自分（相手の共鳴パレットと自分の自己パレットの類似度）の両方向を考慮し、両者が高い組み合わせを「響き合う」と判定する。
