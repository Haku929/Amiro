...
* **AIエンジン**: **Gemini 1.5 Flash** (高速な対話と、会話ログからの性格分析・推論)
...
| **共鳴ベクトル ()** | 相手に求める理想の数値 (5次元) | 自身の「理想」として検索用 |
...
1. **抽出と評価**: 会話終了後、AIが  と  を算出。ユーザーが「自己評価」を行い、保存を決定。
2. **スロットへの定着**: スロットが空いていれば即時保存。満杯の場合は、既存の3つのうち1つを選択して「上書き」。
...
| **ホーム（AIガチャ）** | 今日向き合う「鏡」の選択 | 毎日ランダムな性格のAIが3体出現。日付（UTC）をシードにしたランダム選択で、同一日は同じ3体が表示される。占い感覚で選ばせる。 |
| **AI Mirror チャット** | 自分を引き出す対話 | 10往復限定。AIがユーザーの「特定の顔」を引き出す。 |
| **分人レポート** | 分析結果の提示と保存 | 5角形グラフを表示。「この自分を愛する」なら保存ボタン。 |
| **プロフィール管理** | 3つの分人の管理 | 保存済みの自分を確認。3つ満杯ならどれかを捨てて上書き。 |
...
| DELETE | `/api/slots/:slotIndex` | 指定スロット削除 | なし | 204 No Content |
...
| メソッド | パス | 説明 | リクエストボディ | レスポンス |
| --- | --- | --- | --- | --- |
| POST | `/api/ai/analyze` | 会話ログから性格ベクトル・理想ベクトルを抽出 | `{ messages: { role: "user" \| "model", content: string }[] }` | `{ selfVector: Big5Vector, resonanceVector: Big5Vector, personaSummary: string }` |

フロントからは会話終了後に1回だけ呼び、結果を分人レポート画面に渡す想定。
...
| AI・プロンプト | `lib/gemini.ts`, `lib/prompts/`, `app/api/ai/analyze/route.ts` |
...
5. **ホーム・チャット** → UI 基盤と「今日の3体」ロジック・対話用プロンプトに依存。分析 API は不要。
...
* **[P1]** 会話ログを渡して Big5 の selfVector を返すプロンプト・関数の実装。
* **[P1]** 同上で resonanceVector（理想の相手）を返すロジックの実装。
* **[P1]** personaSummary を会話から要約するロジックの実装。
* **[P1]** `/api/ai/analyze` のルートハンドラ実装（会話配列を受け取り、上記3つをまとめて返す）。
...
* **[P1]** `DELETE /api/slots/:slotIndex` の実装。
...
* **[P1]** ホーム（AIガチャ）ページ：3体のAIカード表示と選択遷移。依存: 「今日の3体」ロジック（AI・プロンプト）。
* **[P1]** AI Mirror チャットページ：メッセージリスト、入力欄、10往復制限の表示。依存: 日替わりAIのシステムプロンプト。
* **[P1]** 分人レポートページ：5角形グラフ表示エリア、保存ボタン、スロット選択（満杯時）のUI。依存: `/api/ai/analyze`、チャート、`GET /api/users/me`、`POST /api/slots`。
* **[P1]** プロフィール管理ページ：3スロットの一覧・編集・削除・上書き選択のUI。依存: `GET /api/users/me`、slots 系 API。
...
* **[P1]** プロフィールからスロット削除・上書きした後の再取得と表示更新。
* **[P1]** 共鳴マッチング一覧〜詳細〜（必要なら戻る）の遷移。詳細は P2 なら簡易表示でよい。

### 🧠 AI・プロンプト担当

* **[P1]** 日替わりで出現するAIの性格パターン（プロンプトのバリエーション）を3種類以上用意。
* **[P1]** ホームで「今日の3体」をランダムに選ぶロジックと、選ばれたAIに応じたシステムプロンプトの適用。依存: 日付（UTC）シードの仕様。
* **[P1]** 会話から Big5 を推定するためのプロンプトテンプレート作成（入力: 会話ログ、出力: JSON の selfVector）。
* **[P1]** 理想の相手（resonanceVector）を導くプロンプトテンプレート作成。
* **[P1]** personaSummary 用の要約プロンプト作成。
* **[P1]** 分析結果の妥当性を確認するためのテスト会話パターンと、期待値のメモ（分析精度向上の検証用）。
